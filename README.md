...
In addition, to keep things as simple as possible, I chose to use the built-in HID drivers in Microsoft Windows.  By using the appropriate descriptors in the project, Windows will automaticaly load the driver.  The simple example uses the Vendor-Defined profile.  To check it out, go to 
...
a) To use the newer devices with the ATMEL ICE, the firmware needs to be updated to Version 1.2e.  Microchip Studio support ended some time ago but the tool is still very useful.  But, to update the firmware I found I had to install MPLABX from Microchip and create an empty main.c to force MPLABX to update the firmware.  After that, I could switch to Microchip Studio.  

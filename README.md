...
a) To use the newer devices with the ATMEL-ICE, the ICE firmware needs to be updated to Version 1.2e.  Microchip Studio 7 support ended some time (years) ago but I prefer using it over MPLABX.  However, Studio no longer keeps the firmware packs up to date through the Device Pack Manager.  I found I had to install MPLABX from Microchip, create an empty main.c and load the program to force MPLABX to update the firmware (there might be a way to manually load the firmware pack).

b) I found a very annyoing feature of Microchip Studio is that it continually inspects HID devices.  I couldn't find any reason for this, but if you set the descriptor string indices to anything other than 0, you will find that Studio gets into a loop looking at your device (the setup packet printf shows a continuous roll of string requests). If you want string descriptors, you will need to quit Microchip Studio while testing.  Erk.  Microchip (as of 2026) suggests that they are moving to VS Code, so that is probably the way to go assuming it is an environment issue and I haven't missed something :).
